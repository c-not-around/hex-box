...
namespace HexBoxControl
{
    ...
    public class Cp1251CharConvertor : ICharConverter
    {
        ...
        {
            char c = _Encoding.GetChars(dump, offset, 1)[0];
            return (c < '!') || (c == '\x7f') || (c == '\xa0') || (c == '\xad') ? '\0' : c;
        }
    ...
    public class HexBox : Control
    {
        ...
        {
            int index = _Offset + (p.Y  * _DataColums + p.X) * GetCellBytes();
        ...
        private void DrawLines(Graphics g)
        {
            ...
            for (int row = 0; row < _DataRows; row++)
            {
                ...
                int x = DrawHex(g, Color.Blue, _HeaderLeft, y, _ViewMode == HexBoxViewMode.Words ? offset/2 : offset, 8);
